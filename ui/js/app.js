...
function updateSettingsUI() {
    ...
    document.getElementById('out-speed').innerText = c.speed.toFixed(2) + '×';
    ...
    document.getElementById('out-maxchord').innerText = c.max_chord || '∞';
...
function setupEventListeners() {
    ...
    document.querySelectorAll('#speed-presets button').forEach(b => {
        b.onclick = () => {
            ...
            document.getElementById('out-speed').innerText = appState.config.speed.toFixed(2) + '×';
            configSaveDelay();
        };
    });
...
function renderPlaylist() {
    ...
    list.innerHTML = appState.playlist.map((p, i) => {
        ...
        return `<li class="playlist__item ${active}" data-idx="${i}">
            ...
            <button class="winbtn btn-remove">✕</button>
        </li>`;
    }).join('');
